...
import { newsDataUrl, googleNewsBiharUrl, bhaskarFeedUrl, livehindustanSources } from './districts.js';
import {
  callLLMWithFallback,
  TOKEN_BUDGET,
  estimateTokens,
...
// ── News Sources (sab .env se — lib/districts.js ke helpers) ──
...
export async function fetchAndSaveNews(scheduleSlot) {
  ...
  // Google News RSS
  try {
    const feed = await rssParser.parseURL(googleNewsBiharUrl()).catch(() => null);
    if (feed) {
  ...
  // Bhaskar
  try {
    const res = await fetch(bhaskarFeedUrl(), { headers: { 'user-agent': 'Bihar-Dashboard/1.0' }, signal: AbortSignal.timeout(8000) });
    if (res.ok) {
  ...
  const lhResults = await Promise.allSettled(
    livehindustanSources().map(async ([name, feedUrl]) => {
