...
/* ═══════════════════════════════════════════════════════════
   RSS feed URLs — sab .env se aate hain
   ------------------------------------------------------------
   .env keys:
     LH_BASE_URL                 → Live Hindustan base (default niche)
     LH_FEED_STATE               → state feed
     LH_FEED_<FEED_SLUG>         → har district ki feed (Ara → LH_FEED_ARA)
     GOOGLE_NEWS_RSS_TEMPLATE    → {query} placeholder wala Google News URL
     GOOGLE_NEWS_RSS_URL         → plain Google News Bihar URL (legacy)
     BHASKAR_BIHAR_RSS_URL       → Dainik Bhaskar Bihar feed
   Koi key na mile to neeche wale defaults use hote hain — isliye code Vercel par
   bina env ke bhi chalta rehta hai; sirf override karna ho to .env me daal do.
   ═══════════════════════════════════════════════════════════ */
const DEFAULTS = {
  base: 'https://api.livehindustan.com/feeds/rss/bihar',
  googleNewsTemplate: 'https://news.google.com/rss/search?q={query}&hl=hi-IN&gl=IN&ceid=IN:hi',
  googleNewsBihar: 'https://news.google.com/rss/search?q=Bihar+politics&hl=hi-IN&gl=IN&ceid=IN:hi',
  bhaskar: 'https://www.bhaskar.com/rss-v1--category-3679.xml',
  newsdata: 'https://newsdata.io/api/1/news',
};

export function lhBaseUrl() {
  return String(process.env.LH_BASE_URL || DEFAULTS.base).replace(/\/+$/, '');
}

/** Env key for a Live Hindustan feed slug: ara → LH_FEED_ARA */
export function feedEnvKey(feedSlug) {
  return `LH_FEED_${String(feedSlug || '').toUpperCase().replace(/[^A-Z0-9]/g, '_')}`;
}

/** State-level (Bihar) Live Hindustan feed URL. */
export function stateFeedUrl() {
  return process.env.LH_FEED_STATE || process.env.HINDUSTAN_BIHAR_RSS_URL || `${lhBaseUrl()}/rssfeed.xml`;
}

/**
 * Live Hindustan feed URL for a district entry (ya feed slug / district slug).
 * .env me LH_FEED_<SLUG> ho to wahi, warna base + slug se banaya jata hai.
 */
export function districtFeedUrl(entryOrSlug) {
  let feed = null;
  if (entryOrSlug && typeof entryOrSlug === 'object') feed = entryOrSlug.feed;
  else if (typeof entryOrSlug === 'string') {
    const entry = resolveDistrict(entryOrSlug);
    feed = entry ? entry.feed : entryOrSlug;
  }
  if (!feed) return stateFeedUrl();
  return process.env[feedEnvKey(feed)] || `${lhBaseUrl()}/${feed}/rssfeed.xml`;
}

/** Google News RSS search URL — .env ka template + district query. */
export function googleNewsSearchUrl(query) {
  const tpl = process.env.GOOGLE_NEWS_RSS_TEMPLATE || DEFAULTS.googleNewsTemplate;
  const q = encodeURIComponent(query);
  return tpl.includes('{query}') ? tpl.replace('{query}', q) : `${tpl}${q}`;
}

export function googleNewsBiharUrl() { return process.env.GOOGLE_NEWS_RSS_URL || DEFAULTS.googleNewsBihar; }
export function bhaskarFeedUrl() { return process.env.BHASKAR_BIHAR_RSS_URL || DEFAULTS.bhaskar; }
export function newsDataUrl() { return process.env.NEWSDATA_URL || DEFAULTS.newsdata; }

/** Live Hindustan sources for the general news pipelines: [label, url, slug]. */
export function livehindustanSources() {
  return [
    ['बिहार', stateFeedUrl(), null],
    ...LIVEHINDUSTAN_FEEDS.map((d) => [d.hi, districtFeedUrl(d.feed), d.feed]),
  ];
}

/** UI dropdown list for /api/rss-news: { slug, label, url }. */
export function livehindustanFeedList() {
  return LIVEHINDUSTAN_FEEDS.map((d) => ({ slug: d.feed, label: d.hi, en: d.en, url: districtFeedUrl(d.feed) }));
}
