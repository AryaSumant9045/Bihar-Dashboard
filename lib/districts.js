...
/** Unique district entries (west_champaran ek hi baar). */
export const DISTRICTS_UNIQUE = DISTRICT_SLUGS.map((slug) => DISTRICTS.find((d) => d.slug === slug));
...
/* ═══════════════════════════════════════════════════════════
   RSS feed URLs — HARDCODED (single source of truth)
   ------------------------------------------------------------
   Feed URL badalni ho to bas yahan badlo (ya DISTRICTS ke `feed` slug me).
   .env me koi feed URL nahi rakha jata.
   ═══════════════════════════════════════════════════════════ */
export const LH_FEED_BASE = 'https://api.livehindustan.com/feeds/rss/bihar';
export const GOOGLE_NEWS_SEARCH_BASE = 'https://news.google.com/rss/search';
export const GOOGLE_NEWS_BIHAR_URL = `${GOOGLE_NEWS_SEARCH_BASE}?q=Bihar+politics&hl=hi-IN&gl=IN&ceid=IN:hi`;
export const BHASKAR_BIHAR_FEED_URL = 'https://www.bhaskar.com/rss-v1--category-3679.xml';
...
export function stateFeedUrl() {
  return `${LH_FEED_BASE}/rssfeed.xml`;
}

/**
 * Live Hindustan feed URL for a district entry (ya feed slug / district naam).
 * Hardcoded: base + feed slug, jaise ara → …/bihar/ara/rssfeed.xml
 */
export function districtFeedUrl(entryOrSlug) {
  ...
  return feed ? `${LH_FEED_BASE}/${feed}/rssfeed.xml` : stateFeedUrl();
}

/** Google News RSS search URL (district-wise fallback source). */
export function googleNewsSearchUrl(query) {
  return `${GOOGLE_NEWS_SEARCH_BASE}?q=${encodeURIComponent(query)}&hl=hi-IN&gl=IN&ceid=IN:hi`;
}

export function googleNewsBiharUrl() { return GOOGLE_NEWS_BIHAR_URL; }
export function bhaskarFeedUrl() { return BHASKAR_BIHAR_FEED_URL; }
export function newsDataUrl() { return NEWSDATA_API_URL; }
...
/** UI dropdown list for /api/rss-news: { slug, label, en, url }. */
...
/** Per-district feed slug → { slug, hi, en, url } (buttons/UI ke liye). */
export function districtFeedEntries() {
  return LIVEHINDUSTAN_FEEDS.map((d) => ({ slug: d.feed, districtSlug: d.slug, hi: d.hi, en: d.en, url: districtFeedUrl(d.feed) }));
}
