...
export async function generateExecutiveSummary(item) {
    ...
    try {
        ...
        console.warn(`Gemini unavailable for ${item.id || item.title}; trying Groq fallback:`, geminiError.message);
        try {
            analysis = await analyzeWithGroq(item);
        } catch (groqError) {
            console.warn(`Groq unavailable for ${item.id || item.title}; trying PlugSky fallback:`, groqError.message);
...
async function analyzeWithGroq(item) {
    const apiKey = process.env.GROQ_API_KEY;
    if (!apiKey) throw new Error('GROQ_API_KEY not set');
    const endpoint = `${(process.env.GROQ_API_URL || 'https://api.groq.com/openai/v1').replace(/\/$/, '')}/chat/completions`;
