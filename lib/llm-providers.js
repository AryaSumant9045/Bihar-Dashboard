...
/** ─── Gemini (direct REST — SDK thinkingConfig ignore kar raha tha) ─────── */
async function callGemini(systemPrompt, userPrompt, attempt = 0, jsonMode = true, maxTokensOverride = null) {
  ...
  const body = {
    ...
    generationConfig: {
      temperature: 0.2,
      maxOutputTokens: maxTokensOverride || TOKEN_BUDGET.MAX_OUTPUT_TOKENS,
      /* JSON mode me thinking OFF — warna thinking output budget kha jata hai
         aur JSON aadha (truncated) aata hai. */
  ...
  /* Truncated output par throw nahi — caller repair/partial-accept kar leta hai. */
  const truncated = Boolean(candidate?.finishReason && candidate.finishReason !== 'STOP');
...
/** Kitni Groq keys configured hain (debug/log ke liye). */
export function groqKeyCount() { return groqKeys().length; }
...
async function callGroqWithKey(systemPrompt, userPrompt, jsonMode, modelOverride, maxTokensOverride, apiKey) {
  ...
  if (response.status === 429) {
    ...
    const limitInfo = (() => {
      try { const j = JSON.parse(body); return (j?.error?.message || '').slice(0, 200); } catch { return body.slice(0, 160); }
    })();
    const err = new Error(`Groq rate limit (429)${response.headers.get('retry-after') ? ` retry-after=${response.headers.get('retry-after')}` : ''}${limitInfo ? ` :: ${limitInfo}` : ''}`);
    err.isRateLimit = true;
 ...
 *   geminiMaxOutputTokens?: number  Gemini ke liye alag cap (bada JSON uske liye theek)
 */
export async function callLLMQuick(systemPrompt, userPrompt, opts = {}) {
  ...
  if (!preferGroq && geminiAvailable()) {
    try {
      return await callGemini(systemPrompt, userPrompt, 0, jsonMode, opts.geminiMaxOutputTokens || opts.maxOutputTokens);
    } catch (geminiErr) {
