...
/** ─── Groq ──────────────────────────────────────────────── */
async function callGroq(systemPrompt, userPrompt, jsonMode = true, modelOverride = null) {
...
export async function callLLMWithFallback(systemPrompt, userPrompt) {
  ...
  if (geminiAvailable()) try {
    ...
    if (isRateLimit(geminiErr1) && geminiAvailable()) {
...
export async function callLLMQuick(systemPrompt, userPrompt, opts = {}) {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      return await callGroq(systemPrompt, userPrompt, jsonMode, opts.model);
    } catch (groqErr) {
